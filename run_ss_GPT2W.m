...
solutionSep.PARAMS.subsetGroupSize = 1;
...
GPT2W_epochs_filename = 'D:\PHD_Research\output\GPT2W_epoch.mat';
GPT2W_pos_all = cat(2, outData.pos);
GPT2W_pl_all  = cat(2, outData.pl);
GPT2W_trops_all  = solutionSep.aivFilter.tropsAvg;
GPT2W_epoch_all  = cat(2, outData.epoch);
save(GPT2W_pos_filename,'GPT2W_pos_all');
save(GPT2W_pl_filename,'GPT2W_pl_all');
save(GPT2W_trops_filename,'GPT2W_trops_all');
save(GPT2W_epochs_filename,'GPT2W_epoch_all');

%% test measurements
idx_all = [];
for i = 1:1:size(gnssMeas.range.obs,3)
    cat(1, idx_all,find(any(gnssMeas.range.obs(:,:,i))));
end
