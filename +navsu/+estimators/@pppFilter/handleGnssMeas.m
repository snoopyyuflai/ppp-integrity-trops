...
p.addParameter('SimpleModel',true);
...
% SNR masking/ Singal Noise Ratro (CN0)
...
if nMeas > 0
    if isempty(extraInputs)
        ...
        % map prn id to contellation id, and also remove the repeated ones
        ...
        tRx = epoch-obj.clockBias(ib)./c; % Rough Transmission Time
...
%             llhi = navsu.geo.xyz2llh(pos');
            ...
            if abs(llhi(3)) > 1e5
                ...
                [trop, m, ~] = navsu.ppp.models.tropDelay(el*180/pi, az*180/pi, ...
                    llhi(:,3), llhi(:,1), llhi(:,2), doy, PARAMS, [], [], epoch, obj.tropsPreloadData);
                obj.tropsAvg = cat(1,obj.tropsAvg, mean(trop,1));
            end
            % TEC for each LOS
    ...
    for idx = 1:nMeas
        ...
        predMeas(idx) = predMeasi; % predicted measurements, pseudorange
        H(idx,:) = Hii;            % Sensitivity
        R(idx,idx) = weighti*sigMeasi^2; % Measurement Sigma
    end
    
    % Remove measurements from satellites that are too low (elevation angle)
