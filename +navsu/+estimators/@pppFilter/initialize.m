...
p.addParameter('tropsPreloadData',[]);

% parse the results
parse(p, varargin{:});
res        = p.Results;
measExclude = res.measExclude;
tropsPreloadData = res.tropsPreloadData;
%% Initialize grid points if the tropospherical model chosen needs 
switch obj.PARAMS.tropModel
    case 'GPT2W_1'
        if(isempty(tropsPreloadData))
            % tropsGrid is empty, load the grid files
            obj.tropsPreloadData = navsu.readfiles.loadGPT2W_1(obj.PARAMS.tropModel_FileName);
        else
            obj.tropsPreloadData = tropsPreloadData;
        end
    case 'NoDelay'
        % exclude tropospheric delay model from the state
        obj.PARAMS.states.trop = false;
        obj.PARAMS.SIGMA0.TROP = 0;
        obj.PARAMS.Q.TROP = 0;
    otherwise
end
