function pl_hi = chi2_approx1_vpl(sigma_0, sigma_ss, p_subset, Kchi2, phmi,tol)
%*************************************************************************
...
while (abs(pl_hi-pl_lo)>tol)&&(count<100)
   ...
   phmi_mid = ss_approx1_phmi(sigma_0, sigma_ss, p_subset, T, pl_mid);
   if phmi_mid>phmi
       pl_lo = pl_mid;
   else
       pl_hi = pl_mid;
